...
| 能力 | 入口 | 说明 |
|------|------|------|
| 对话创作 | 顶栏「对话创作」/ `/chat` | 对话式收集需求、大纲与风格，确认后创建生成任务 |
| 美化 PPT | 「美化 PPT」/ `/jobs/beautify` | 上传 `.pptx` 并选择内置模板，走 beautify 流水线 |
| 创建 | 「创建」/ `/jobs/new` | 从主题或文档结构化创建全新演示文稿 |
| ForgeBot 助手 | 右下角机器人 | 任务进度播报；空闲时引导创作入口 |
| 主题换肤 | 顶栏调色板图标 | 6 套皮肤 + 明暗模式，即时切换 |
| 管理后台 | `/admin` | 用户、任务、Docker Runner、LLM 模型配置 |

作品列表支持状态筛选、搜索、分页；卡片悬停可预览、下载、编辑、删除。
...
左侧会话栏 + 右侧创作区，通过多轮对话确认需求、大纲与视觉风格后一键生成。

### 美化 PPT

![美化 PPT](./images/preview-beautify.png)

上传已有 `.pptx`，从内置模板库选择 deck/layout，提交后按模板美化流水线执行。

### 创建

![创建任务](./images/preview-create.png)

结构化表单：内容源、视觉调性、素材策略与高级选项；支持 AI 智能填充（需在 Admin 配置 LLM）。
